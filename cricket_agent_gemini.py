...
match_agent = Agent(
    ...
    model=Gemini(id="gemini-2.5-flash"),  # Using OpenAI GPT-4o
    tools=[DuckDuckGo()],
    instructions=[
        "Search for live cricket match scores RCB vs SRH",
...
player_agent = Agent(
    ...
    instructions=[
        ...
        "Include batting and bowling stats for IPL 2026 matches.",
...
cricket_team.print_response(
    "Create summarize table to Get the latest score of the India vs south africa  match, recent performance for tilak verma, and cricket news.",
    stream=True
)